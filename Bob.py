...
def here_now_callback(result, status):
    ...
    for channel_data in result.channels:
        for occupant in channel_data.occupants:
            if(occupant.uuid == "Alice" and TxID == 0):# Alice is here lets start
                generateGensisBlock("Bob") # Generate Gensis Block
...
class MySubscribeCallback(SubscribeCallback):  
    def presence(self, pubnub, presence): #Activiated when a UUID is connected/leaves
        print("presense",presence.uuid, presence.event) 
        #Check occupants for Alice
    ...
    def message(self, pubnub, message):
        ...
        if type(msg) == dict:
            if msg['sender'] != pnconfig.uuid:
                print(str(TxID+1)+": Alice: ", msg['content']) 
                #save incoming message
                processTransaction(msg['content'])
                #Send Operation
                if TxID < 9:
                    generateTransaction("Bob")
        if(TxID >= 9):
            unsub() #Exit program 
...
def processTransaction(data): #Validates JSON file & Stores it
    ...
    #---validate---
    valid = True
    file_in = open(python_file_dir+"/Bob/Block"+ str((TxID)) + ".json", "r") 
    ...
    if(int(separated_data[0]) != (TxID+1)):
        valid = False
        print("Error: Alice's Transaction number "+str(separated_data[0])+" is wrong!")
    
    #Validate Hash/Nonce
    if(str(separated_data[1]) != str(temp_hash.hexdigest())):
        valid = False
        print("Error: Alice's Hash/Nonce "+str(separated_data[1])+" doesn't match! "+str(temp_hash.hexdigest()))

    #Validate Transaction
    if(int(separated_data[3]) in exclude): 
        valid = False
        print("Error: Alice's choosen position "+str(separated_data[3])+" is already taken!")
    ...
    if valid:
        ...
        #Format
        next_block = json_format # Copies the formating of JSON
        Transaction = ["Alice", separated_data[3]] #<Transaction>
        ...
        file_out = open(python_file_dir+"/Bob/Block"+str(separated_data[0])+ ".json", "w")
        file_out.write(next_block)
        file_out.close()
    else:
        unsub() #Exit program
        os._exit(-1) # End of Game

def randomNumber(): #Computes Random Number
    global exclude
    true_random = SystemRandom().randrange(1,10) # 1-9 (exclusive of 9)
    while true_random in exclude: #if in exclusion list find new random
        true_random = SystemRandom().randrange(1,10) # 0-9 (exclusive of 9)
